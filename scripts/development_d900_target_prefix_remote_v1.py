"""Scan the remote held-out d900 Stage5 partition; transfer only summary JSON."""
...
OUTPUT = ROOT / "results/responsive-team-v4/v50-d900-prefix-target-choice-validation.json"
...
def main() -> None:
    ...
    node = "node004"
    relative = "o2o_dps/upper_kara_target_prefix_validation_v1.py"
    subprocess.run(
    ...
    source = (
        "import json; from o2o_dps.upper_kara_target_prefix_validation_v1 "
        "import analyze_heldout_instance_gzip_v1; "
        f"print(json.dumps(analyze_heldout_instance_gzip_v1({STAGE5!r}), ensure_ascii=False))"
    )
    command = (
        f"cd {shlex.quote(REMOTE)} && "
        "/home/zhengliang01/scheduleurm_work/conda_envs/scomp-py310/bin/python3.10 "
        f"-c {shlex.quote(source)}"
    )
    rc, stdout, stderr = scheduler.run_on(node, command, timeout=240, check=False)
    if rc != 0:
        raise RuntimeError(f"remote target-prefix diagnostic failed (rc={rc}): {stderr}")
