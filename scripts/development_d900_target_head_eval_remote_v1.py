"""Run exact head-denominator validation on node004; pull only small JSON."""
...
OUTPUT = ROOT / "results/responsive-team-v4/v51-d900-exact-target-head-denominator.json"
...
def main() -> None:
    ...
    source = (
        "import json; from o2o_dps.upper_kara_target_head_heldout_eval_v1 "
        "import evaluate_heldout_stage5_gzip_v1; "
        f"print(json.dumps(evaluate_heldout_stage5_gzip_v1({STAGE5!r}), ensure_ascii=False))"
    )
    command = (
    ...
    if rc != 0:
        raise RuntimeError(f"remote exact target-choice gate failed (rc={rc}): {stderr}")
