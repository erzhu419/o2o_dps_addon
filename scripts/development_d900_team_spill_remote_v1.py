"""Run the d900 team spill diagnostic remotely; pull its small JSON only."""
...
OUTPUT = ROOT / "results/responsive-team-v4/v46-d900-early-team-spill-diagnostic.json"
...
def main() -> None:
    ...
    node = "node004"
    relative = "scripts/development_d900_team_spill_probe_v1.py"
    subprocess.run(
    ...
    if rc != 0:
        raise RuntimeError(f"remote team spill probe failed (rc={rc}): {stderr}")
    result = json.loads(stdout)
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"output": str(OUTPUT), "teammate_actor_count": len(result["first_teammate_actor_event_offsets_ms"])}, ensure_ascii=False))
