...
if ([string]::IsNullOrWhiteSpace($Contract)) {
    $Contract = Join-Path $projectRoot 'configs\hpc\dynamic_v5.example.json'
}
if ([string]::IsNullOrWhiteSpace($Bridge)) {
    $Bridge = Join-Path $projectRoot 'bin\o2obridge.seedfix-v11.withdb.goamd64v1.linux-amd64'
}
...
$arguments = @(
    ...
    '-B',
    '-m',
    'o2o_dps.hpc_dynamic_environment_v5',
    '--site-config',
    $SiteConfig,
    '--contract',
    $Contract,
