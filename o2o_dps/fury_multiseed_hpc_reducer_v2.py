"""Streaming reducer for native dynamic-v5 diagnostic group receipts.

The reducer requires the exact planned group/policy Cartesian product and
revalidates every runner-v4 lane result against its producer.  Incomplete or
omitted lanes remain in the aggregate as negative diagnostic outcomes; they do
not become missing data.  Even a complete input set remains simulator-only and
cannot authorize the currently blocked five-policy formal comparison.
"""
...
from .fury_multiseed_hpc_worker_v2 import (
...
REDUCTION_SCHEMA_V2 = "fury_multiseed_dynamic_v5_hpc_reduction/v2"


class FuryMultiseedHpcReducerV2Error(RuntimeError):
    """The diagnostic worker output set is missing, conflicting, or invalid."""
...
def _read_json(path: Path, label: str) -> JSONMap:
    try:
        ...
        raise FuryMultiseedHpcReducerV2Error(f"could not read {label}: {error}") from error
    if not isinstance(value, dict):
        raise FuryMultiseedHpcReducerV2Error(f"{label} must be an object")
    return value
    ...
    try:
        with path.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, 1):
                if not line.strip():
                    raise FuryMultiseedHpcReducerV2Error(
                ...
                rows.append(value)
    except FuryMultiseedHpcReducerV2Error:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise FuryMultiseedHpcReducerV2Error(
...
def _validate_group_receipt(
    ...
    *,
    ...
    if (
        ...
        or receipt.get("schema") != GROUP_RECEIPT_SCHEMA_V2
        or receipt.get("status") != "COMPLETE_SIMULATOR_ONLY_NONVOTING"
    ):
        raise FuryMultiseedHpcReducerV2Error("group receipt field set or schema mismatch")
    ...
    exact = {
        ...
        "result_count": len(ALLOWED_POLICY_IDS),
    ...
    for field, expected in exact.items():
        if receipt.get(field) != expected:
            raise FuryMultiseedHpcReducerV2Error(f"group receipt {field} mismatch")
    if len(rows) != len(ALLOWED_POLICY_IDS):
        raise FuryMultiseedHpcReducerV2Error("group rollout count mismatch")
    ...
    if len(scenarios) != 1:
        raise FuryMultiseedHpcReducerV2Error("group scenario is not unique")
    ...
    for row in rows:
        ...
        if not isinstance(policy_identity, Mapping):
            raise FuryMultiseedHpcReducerV2Error("rollout policy identity is missing")
        policy_id = policy_identity.get("policy_id")
        if policy_id not in policies:
            raise FuryMultiseedHpcReducerV2Error("rollout policy is not planned")
        try:
        ...
        except FuryMultiseedHpcWorkerV2Error as error:
            raise FuryMultiseedHpcReducerV2Error(str(error)) from error
        observed_policy_ids.append(str(policy_id))
    if tuple(observed_policy_ids) != ALLOWED_POLICY_IDS:
        raise FuryMultiseedHpcReducerV2Error(
            "group rows are not the exact ordered diagnostic policy set"
        )
...
def reduce_dispatch_v2(
    runner_plan: Mapping[str, Any],
    dispatch: Mapping[str, Any],
    *,
    output_directory: str | Path,
) -> JSONMap:
    """Reduce the complete diagnostic group set without formal scoring."""
    ...
    if sorted(node_by_group) != expected_group_ids:
        raise FuryMultiseedHpcReducerV2Error("dispatch group set differs from runner plan")

    by_policy: dict[str, JSONMap] = {
        policy_id: _new_stats() for policy_id in ALLOWED_POLICY_IDS
    }
    by_scenario_policy: dict[tuple[str, str, str], JSONMap] = defaultdict(_new_stats)
    completion_modes: dict[str, Counter[str]] = {
        policy_id: Counter() for policy_id in ALLOWED_POLICY_IDS
    }
    pair_stats = {
        "candidate_minus_cat": {
            "pair_count": 0,
    ...
    for group_id in expected_group_ids:
        ...
        if result_file != f"groups/{group_id}.jsonl":
            raise FuryMultiseedHpcReducerV2Error("group result path mismatch")
        ...
        for label, baseline_id in (
            ...
            pair["both_offline_score_eligible_count"] += int(
                by_id[baseline_id]["lane_result"]["offline_score_eligible"]
                is True
                and by_id[CAT2NEW_POLICY_ID]["lane_result"][
    ...
    return {
        ...
        "heavy_execution_started": (
            checked_dispatch["execution_kind"] != "SINGLE_NODE_DIAGNOSTIC_FIXTURE"
        ),
        "live_fidelity": False,
        "comparison_ready": False,
        "scientific_result_available": False,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except FuryMultiseedHpcReducerV2Error as error:
